---
title: "CHIR HIV RV306 Vaginal - Longitudinal study Group 2"
...
We are interested in comparing samples the evolution of microbiome composition and function in a specific treatment group to answer the question how is the microbiome altered depending on the treatment?. Patients on group 2 received the vaccination regime of RV144, and an additional boost of both ALVAC and AIDSVAX at week 48.
...
mre_2 <- 
  metar::get_meta(mre) %>%
  dplyr::filter(Treatment_group_1 %in% c("t_2")) %>%
  dplyr::pull(SampleID) %>%
...
mre_2 contains all samples from group 2, which were collected at time-points 0, 14, 26 and 50 and received vaccination on time-points 0, 4, 12, 24 and 48. They are 144 samples from 38 patients. In this group, there are `r sum(mre_2@metadata@metadata_df$Arm=="Placebo")` samples in the placebo arm and `r sum(mre_2@metadata@metadata_df$Arm=="Active")` samples in the active arm.

![Group 2](assets/Study_2.png)
...
## run rarefaction
mre_2 <- metar::virgo_rarefaction(mre_2, save_files = FALSE)
...
metar::get_cat(mre_2) %>%
  ...
  purrr::walk(~ {
    cat('\n\n### `', .x, '`\n\n')
    metar::get_diversity(mre_2, type = "virgo", res_slot = "rarefaction") %>%
...
In these graphs, we can see the changes in the relative gene richness over time of samples separated by categorical variable groups. It seems that microbiome stability is higher in the active arm compared to the placebo arm. Gene richness also increases in variability in the **G. Vaginalis** dominated group.

## PCoA based on Bray Curtis 

```{r PCoA, fig.width=20, fig.height=5}
ps <- metar::get_phyloseq(mre_2, type = "virgo")
...
## run barplots 
mre_2 <- metar::virgo_barplots(mre_2, top_n = 50, save_files = FALSE)
...
## run heatmap
mre_2 <- metar::virgo_heatmap(mre_2, top_n = 50, save_files = FALSE)
...
In this hierarchical clustering heatmap, we can clearly see the three groups. It can be seen that the vaginotype **G. Vaginalis** is enriched in this specie, while the other two are dominated by **L. Crispatus** and **L. Iners**.
...
```{r adonis_table}
mre_2 <- metar::virgo_nmds(mre_2, top_n = 50, save_files = FALSE)

metar::get_taxa(mre_2, "virgo", "nmds") %>%
  purrr::pluck("ta1", "top_50", "all_adonis") %>%
  dplyr::arrange(Pr..F.)
```

The Adonis test indicates that there are significant differences in the taxonomic composition between the levels of the categorical variables: **pass_reads_quantile**, **vaginotype** and **vaginotype_condition**. Surprisingly, there are also significant differences in the taxonomic composition between samples form different medical centers.
...
# run ancom
mre_2 <- metar::virgo_ancom(mre_2, save_files = FALSE)

# plot ancom
metar::get_cat(mre_2) %>%
  dplyr::pull(1) %>%
  purrr::walk( ~ {
    plt <- metar::get_taxa(mre_2, "virgo", "ancom") %>%
...
ANCOM plots indicate us that there are abundance values that are significantly different between **medical_center**, **pass_reads_quantile**, **vaginotype** and **vaginotype_condition** groups. Regarding pass_reads_quantile, these differences are bigger when comparing c_4 group against others (but do not occur on c_1 vs c_2 or c_1 vs c_3 and c_2 vs c_3), and regarding vaginotype, these differences are bigger when comparing **L. Crispatus** and **G. Vaginalis** groups.

## Differential Analyisis Lefse {.tabset }

```{r run_lefse, results='asis'}
# run lefse
mre_2 <- metar::virgo_lefse(mre_2, save_files = FALSE)

# plot lefse
metar::get_cat(mre_2) %>%
  dplyr::pull(1) %>%
  purrr::walk(~ {
    plt <- metar::get_taxa(mre_2, "virgo", "lefse") %>%
      purrr::pluck("ta1", .x, "plot")
...
```{r stratum_samples}
sample_counts <- metar::get_meta(mre_2) %>%
...
met <- metar::get_meta(mre_2) %>% 
  dplyr::select(1, Timepoint) %>% 
  dplyr::right_join(mre_2@taxa@virgo@barplots$ta1$top_50$rel_abundance$bray_NMDS1_order_barplot$data, met, by = "SampleID")

plot <- mre_2@taxa@virgo@barplots$ta1$top_50$rel_abundance$bray_NMDS1_order_barplot
...
* At time-points 0, 14 and 26, most of the samples are dominated by **L. iners** or **L. Crispatus**, while some of them are **G. vaginalis** dominated and polimicrobial
...
After performing all these tests, we can conclude that microbiome composition stays stable at group 2.
