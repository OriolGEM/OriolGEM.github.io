---
title: "CHIR HIV RV306 Vaginal - Longitudinal study Group 3"
...
We are interested in comparing samples the evolution of microbiome composition and function in a specific treatment group to answer the question how is the microbiome altered depending on the treatment?. Patients on group 3 received the vaccination regime of RV144, and an additional boost of ALVAC at week 48.
...
mre_3 <- 
  metar::get_meta(mre) %>%
  dplyr::filter(Treatment_group_1 %in% c("t_3")) %>%
  dplyr::pull(SampleID) %>%
...
mre_3 contains all samples from group 3, which were collected at time-points 0, 14, 26 and 50 and received vaccination on time-points 0, 4, 12, 24 and 48. They are 121 samples from 32 patients. In this group, there are `r sum(mre_3@metadata@metadata_df$Arm=="Placebo")` samples in the placebo arm and `r sum(mre_3@metadata@metadata_df$Arm=="Active")` samples in the active arm.

![Group 3](assets/Study_3.png)
...
## run rarefaction
mre_3 <- metar::virgo_rarefaction(mre_3, save_files = FALSE)
...
metar::get_cat(mre_3) %>%
  ...
  purrr::walk(~ {
    cat('\n\n### `', .x, '`\n\n')
    metar::get_diversity(mre_3, type = "virgo", res_slot = "rarefaction") %>%
...
The gene richness boxplots indicate that there are no significant differences between groups in terms of gene richness, except when separating by the number of QPass reads by quantiles, where the gene richness on the 4th quantile group is significantly higher. This could be explained due to the fact that the sample depth is higher on this group which favors gene richness. There are also significant differences between vaginotype groups. Remarkably, there are significant divverences not only between **G. vaginalis** profile and the other Lactobacillus profiles but also between **L. iners** and **L. crispatus** profile.
...
In these graphs, we can see the changes in the relative gene richness over time of samples separated by categorical variable groups.

## PCoA based on Bray Curtis 

```{r PCoA, fig.width=20, fig.height=5}
ps <- metar::get_phyloseq(mre_3, type = "virgo")
...
In the PCoA plots, it can be seen that the compositional variables that affect each cluster. PCo 1 and 2 explain the 79.4% of the variability (53.9% and 25.5% respectively), while PCo3 explains the 9.7% of the variability. From the plots we can see that PCo1 is the one that explains the separation of cluster 1 (L. Crispatus) from cluster 2 (G. Vaginalis) and cluster 3 (L. Iners), while PCo2 explains the separation of cluster 3 from cluster 1 and 2, even though this separation is not very clear. On the other hand, PCo3 is not useful to discriminate between clusters. It can also be seen that there are very few placebos, all of them belonging to cluster 1.
...
## run barplots 
mre_3 <- metar::virgo_barplots(mre_3, top_n = 50, save_files = FALSE)
...
## run heatmap
mre_3 <- metar::virgo_heatmap(mre_3, top_n = 50, save_files = FALSE)
...
In this hierarchical clustering heatmap, we can clearly see the three groups. We can see how the **G. vaginalis** vaginotype group is highly polymicrobial.
...
```{r adonis_table}
mre_3 <- metar::virgo_nmds(mre_3, top_n = 50, save_files = FALSE)

metar::get_taxa(mre_3, "virgo", "nmds") %>%
...
The nmds plots also indicate that there are significant differences between the levels of the categorical variables: **pass_reads_quantile**, **vaginotype**, and **vaginotype_condition**. Nevertheless, the differences between groups in the variable **Arm** are not seen here.

## Differential Analyisis ANCOM {.tabset }

```{r run_ancom, results='asis'}
# run ancom
mre_3 <- metar::virgo_ancom(mre_3, save_files = FALSE)

# plot ancom
metar::get_cat(mre_3) %>%
  dplyr::pull(1) %>%
  purrr::walk( ~ {
    plt <- metar::get_taxa(mre_3, "virgo", "ancom") %>%
...
ANCOM plots indicate us that there are abundance values that are significantly different between **pass_reads_quantile**, **vaginotype** and **vaginotype_condition** groups. Regarding pass_reads_quantile, these differences are bigger when comparing c_4 group against others, and regarding vaginotype, these differences are bigger when comparing **L. Crispatus** and **G. Vaginalis** groups. Moreover, some differences in abundance values are also observed in the **Arm** group.

## Differential Analyisis Lefse {.tabset }

```{r run_lefse, results='asis'}
# run lefse
mre_3 <- metar::virgo_lefse(mre_3, save_files = FALSE)

# plot lefse
metar::get_cat(mre_3) %>%
  dplyr::pull(1) %>%
  purrr::walk(~ {
    plt <- metar::get_taxa(mre_3, "virgo", "lefse") %>%
      purrr::pluck("ta1", .x, "plot")
...
If we consider features with LDA scores above 3 as biologically relevant, these plots indicate us that **Lactobacillus Iners**, **Lactobacillus Crispatus** and **Gardnerella Vaginalis** are the species that explain the difference between healthy and vaginosis groups. These are also the species that explain the differences between study arms.
...
```{r stratum_samples}
sample_counts <- metar::get_meta(mre_3) %>%
...
met <- metar::get_meta(mre_3) %>% 
  dplyr::select(1, Timepoint) %>% 
  dplyr::right_join(mre_3@taxa@virgo@barplots$ta1$top_50$rel_abundance$bray_NMDS1_order_barplot$data, met, by = "SampleID")

plot <- mre_3@taxa@virgo@barplots$ta1$top_50$rel_abundance$bray_NMDS1_order_barplot
...
* At time-points 0, 14 and 26, most of the samples are dominated by **L. iners** or **L. Crispatus**, while some of them are **G. vaginalis** dominated, with **A. vaginae** codominance and polimicrobial
* At time-point 50, 2 weeks after the boost, most of the samples are Lactobacillus dominated, either by **L. iners** or **L. crispatus**. Others are **G. vaginalis** dominated and polimicrobial. Surprisingly, some few samples are dominated by other Lactobacillus such as **L. gasseri** or **L. jensii**.
...
After performing all these tests, we can conclude that microbiome composition stays stable at group 3. Differences between study arms are due to the fact that the only four placebo samples are dominated by **L. crispatus**.
