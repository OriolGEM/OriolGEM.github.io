---
title: "CHIR HIV RV306 Vaginal - Longitudinal study Group 4A"
...
mre_4A <- 
  metar::get_meta(mre) %>%
  dplyr::filter(Treatment_group %in% c("t_4A")) %>%
  dplyr::pull(SampleID) %>%
...
mre_4A contains all samples from group 4A, which were collected at time-points 0, 14, 26, 50 and 62 and received vaccination on time-points 0, 4, 12, 24 and 60. They are 102 samples from 23 patients. In this group, there are `r sum(mre_4A@metadata@metadata_df$Arm=="Placebo")` samples in the placebo arm and `r sum(mre_4A@metadata@metadata_df$Arm=="Active")` samples in the active arm.


![Group 4A](assets/Study_4A.png)
...
## run rarefaction
mre_4A <- metar::virgo_rarefaction(mre_4A, save_files = FALSE)

## plot rarefaction
metar::get_cat(mre_4A) %>%
  dplyr::pull(1) %>%
  purrr::walk(~ {
    cat('\n\n### `', .x, '`\n\n')
    metar::get_diversity(mre_4A, type = "virgo", res_slot = "rarefaction") %>%
...
The gene richness boxplots indicate that there are no significant differences between groups in terms of gene richness, except when separating by the number of QPass reads by quantiles, where the gene richness on the 4th quantile group is significantly higher. This could be explained due to the fact that the sample depth is higher on this group which favors gene richness. There are also significant differences between vaginotype groups.
...
In these graphs, we can see the changes in the relative gene richness over time of samples separated by categorical variable groups. It seems that microbiome stability is higher in the active arm compared to the placebo.

## PCoA based on Bray Curtis 

```{r PCoA, fig.width=20, fig.height=5}
ps <- metar::get_phyloseq(mre_4A, type = "virgo")
...
## run barplots 
mre_4A <- metar::virgo_barplots(mre_4A, top_n = 50, save_files = FALSE)
...
## run heatmap
mre_4A <- metar::virgo_heatmap(mre_4A, top_n = 50, save_files = FALSE)
...
In this hierarchical clustering heatmap, we can see the three groups. It can be seen that **G. Vaginalis** group is dominated by **G. vaginalis** and **A. vaginae** and that lactobacillus groups are codominated by other Lactobacillus such as **L. jensenii** and **L. gasseri**.
...
The nmds plots also indicate that there are significant differences between the levels of the categorical variables: **pass_reads_quantile**, **vaginotype**, and **vaginotype_condition**. Differences between study arms and between hospitals are also seen here.

## Differential Analyisis ANCOM {.tabset }

```{r run_ancom, results='asis'}
# run ancom
mre_4A <- metar::virgo_ancom(mre_4A, save_files = FALSE)

# plot ancom
metar::get_cat(mre_4A) %>%
  dplyr::pull(1) %>%
  purrr::walk( ~ {
    plt <- metar::get_taxa(mre_4A, "virgo", "ancom") %>%
...
ANCOM plots indicate us that there are abundance values that are significantly different between **pass_reads_quantile**, **vaginotype** and **vaginotype_condition** groups. Regarding pass_reads_quantile, these differences are bigger when comparing c_4 group against others (but do not occur on c_1 vs c_2 or c_1 vs c_3 and c_2 vs c_3), and regarding vaginotype, these differences are bigger when comparing **L. Crispatus** and **G. Vaginalis** groups. Abundance values are also significantly different between **Medical center** groups and **Arm** groups.

## Differential Analyisis Lefse {.tabset }

```{r run_lefse, results='asis'}
# run lefse
mre_4A <- metar::virgo_lefse(mre_4A, save_files = FALSE)

# plot lefse
metar::get_cat(mre_4A) %>%
  dplyr::pull(1) %>%
  purrr::walk(~ {
    plt <- metar::get_taxa(mre_4A, "virgo", "lefse") %>%
      purrr::pluck("ta1", .x, "plot")
...
If we consider features with LDA scores above 3 as biologically relevant, these plots indicate us that **Lactobacillus Iners**, **Lactobacillus Crispatus** and **Gardnerella Vaginalis** are the species that explain the difference between healthy and vaginosis groups, but also between **Medical center** and study **Arm**.
...
```{r stratum_samples}
sample_counts <- metar::get_meta(mre_4A) %>%
...
met <- metar::get_meta(mre_4A) %>% 
  dplyr::select(1, Timepoint) %>% 
  dplyr::right_join(mre_4A@taxa@virgo@barplots$ta1$top_50$rel_abundance$bray_NMDS1_order_barplot$data, met, by = "SampleID")

plot <- mre_4A@taxa@virgo@barplots$ta1$top_50$rel_abundance$bray_NMDS1_order_barplot
...
* At time-point 62, 2 weeks after the boost, most of the samples are Lactobacillus dominated, either by **L. iners** or **L. crispatus**. Others are **G. vaginalis** dominated and polimicrobial. Surprisingly, some few samples are dominated by other Lactobacillus such as **L. gasseri** or **L. jensii**.
...
After performing all these tests, we can conclude that microbiome composition stays stable at group 4A. Differences between study arms are due to the fact that the placebo samples are dominated by **L. crispatus** and **L. iners**.
