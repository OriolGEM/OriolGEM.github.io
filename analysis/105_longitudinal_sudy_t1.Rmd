---
title: "CHIR HIV RV306 Vaginal - Longitudinal study Group 1"
...
We are interested in comparing samples the evolution of microbiome composition and function in a specific treatment group to answer the question how is the microbiome altered depending on the treatment?. Patients on group 1 received the vaccination regime of RV144, without any boosts.
...
mre_1 <- 
  metar::get_meta(mre) %>%
  dplyr::filter(Treatment_group_1 %in% c("t_1")) %>%
  dplyr::pull(SampleID) %>%
...
mre_1 contains all samples from group 1, which were collected at time-points 0, 14, 26 and 50 and only received vaccination on time-points 0, 4, 12 and 24. They are 40 samples from 10 patients. In this group, there are `r sum(mre_1@metadata@metadata_df$Arm=="Placebo")` samples in the placebo arm and `r sum(mre_1@metadata@metadata_df$Arm=="Active")` samples in the active arm.

![Group 1](assets/Study_1.png)
...
## run rarefaction
mre_1 <- metar::virgo_rarefaction(mre_1, save_files = FALSE)
...
metar::get_cat(mre_1) %>%
  ...
  purrr::walk(~ {
    cat('\n\n### `', .x, '`\n\n')
    metar::get_diversity(mre_1, type = "virgo", res_slot = "rarefaction") %>%
...
ps <- metar::get_phyloseq(mre_1, type = "virgo")
...
In the PCoA plots, it can be seen that the compositional variables that affect each cluster. PCo 1 and 2 explain the 84.4% of the variability (57.9% and 26.5% respectively), while PCo3 only explains the 8% of the variability. No clear cluster differentiation can be seen. Moreover, there are no placebo samples on this group.
...
## run barplots 
mre_1 <- metar::virgo_barplots(mre_1, top_n = 50, save_files = FALSE)
...
## run heatmap
mre_1 <- metar::virgo_heatmap(mre_1, top_n = 50, save_files = FALSE)
...
```{r adonis_table}
mre_1 <- metar::virgo_nmds(mre_1, top_n = 50, save_files = FALSE)

metar::get_taxa(mre_1, "virgo", "nmds") %>%
...
# run ancom
mre_1 <- metar::virgo_ancom(mre_1, save_files = FALSE)

# plot ancom
metar::get_cat(mre_1) %>%
  dplyr::pull(1) %>%
  purrr::walk( ~ {
    plt <- metar::get_taxa(mre_1, "virgo", "ancom") %>%
...
ANCOM plots indicate us that there are abundance values that are significantly different between **pass_reads_quantile**, **vaginotype** and **vaginotype_condition** groups, but not between **Treatment_group** groups. Regarding pass_reads_quantile, these differences are bigger when comparing c_4 group against others , and regarding vaginotype, these differences are bigger when comparing Lactobacillus-dominated and **G. Vaginalis** groups.

## Differential Analyisis Lefse {.tabset }

```{r run_lefse, results='asis'}
# run lefse
mre_1 <- metar::virgo_lefse(mre_1, save_files = FALSE)

# plot lefse
metar::get_cat(mre_1) %>%
  dplyr::pull(1) %>%
  purrr::walk(~ {
    plt <- metar::get_taxa(mre_1, "virgo", "lefse") %>%
      purrr::pluck("ta1", .x, "plot")
...
```{r stratum_samples}
sample_counts <- metar::get_meta(mre_1) %>%
...
met <- metar::get_meta(mre_1) %>% 
  dplyr::select(1, Timepoint) %>% 
  dplyr::right_join(mre_1@taxa@virgo@barplots$ta1$top_50$rel_abundance$bray_NMDS1_order_barplot$data, met, by = "SampleID")

plot <- mre_1@taxa@virgo@barplots$ta1$top_50$rel_abundance$bray_NMDS1_order_barplot
...
* At time-point 0 and 14, most of the samples are dominated by **L. iners**, while some of them are **G. vaginalis** dominated and polimicrobial
* At time-point 26, most of the samples are **G. vaginalis** dominated and polimicrobial. Other samples are dominated by **L. iners**.
* At time-point 50, 26 weeks after the last vaccine shoot, most of the samples are Lactobacillus dominated, either by **L. iners** or **L. crispatus**. Others are **G. vaginalis** dominated and polimicrobial.
...
After performing the longitudinal study, we can conclude that microbiome composition remains stable at group 1, which does not receive any vaccine boost after following the RV144 regime.
